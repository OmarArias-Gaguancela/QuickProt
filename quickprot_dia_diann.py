# -*- coding: utf-8 -*-
"""QuickProt-DIA-DIANN.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/15IS_c4V1nw8jLsMhk4hhVLKXA2kdObrk

<img src="https://raw.githubusercontent.com/OmarArias-Gaguancela/LOGOS_OAG/main/QuickProt DIA.jpg" width="180" height="100">

# **QuickProt-DIA (DIA-NN)**: This pipeline aims to analyze and visualize DIA proteomics data using the DIA-NN output table.
...
<img src="https://raw.githubusercontent.com/OmarArias-Gaguancela/LOGOS_OAG/main/QuickProt_DIA_DIA_NN_input_output.jpg" width="380" height="200">
...
def on_button_click(b):
    ...
    display(widgets.HTML("<h3>Do you want to rename your samples?</h3>"))
    yes_btn = widgets.Button(description="Yes", style={'button_color': '#4CAF50'})
    no_btn = widgets.Button(description="No", style={'button_color': '#F44336'})
    ...
    def rename_samples():
        ...
        for sample in selected_samples:
            ...
            inputs.append(widgets.Text(value=trimmed, description=sample, layout=widgets.Layout(width='auto', description_width='initial')))
        input_box = widgets.VBox(inputs)

        def save_names(_):
            name_map = {inp.description: inp.value for inp in inputs}
            filtered_df['Sample'] = filtered_df['Sample'].map(name_map)
            assign_groups(filtered_df)

        save_btn = widgets.Button(description="Save Sample Names", style={'button_color': '#4CAF50'})
        save_btn.on_click(save_names)
        display(input_box, save_btn)

    def skip_rename(_):
        assign_groups(filtered_df)

    yes_btn.on_click(lambda b: rename_samples())
    no_btn.on_click(skip_rename)
    display(yes_btn, no_btn)

def assign_groups(filtered_df):
    ...
    group_input = widgets.Text(placeholder='Enter group name', description='Group name:')
    group_selector = widgets.SelectMultiple(options=filtered_df['Sample'].unique().tolist(), description='Samples:')
    assigned = set()

    def add_group(_):
        group = group_input.value
        selected = list(group_selector.value)
        filtered_df.loc[filtered_df['Sample'].isin(selected), 'Experimental groups'] = group
        assigned.update(selected)
        group_selector.options = [s for s in filtered_df['Sample'].unique() if s not in assigned]
        group_input.value = ''

    def save_csv(_):
        filtered_df.to_csv('TABLES/report_updated.csv', index=False)
        display(widgets.HTML("<b>CSV file saved to TABLES/report_updated.csv</b>"))

    add_btn = widgets.Button(description="Add Group", style={'button_color': '#4CAF50'})
    add_btn.on_click(add_group)
    save_btn = widgets.Button(description="Save CSV", style={'button_color': '#4CAF50'})
    save_btn.on_click(save_csv)

    display(group_input, group_selector, add_btn, save_btn)

# Display UI
display(widgets.HTML("<h3>Select samples to analyze</h3>"))
display(sample_checkboxes_box)
submit_btn = widgets.Button(description="Submit", style={'button_color': '#4CAF50'})
...
def filter_dataframe(min_peptides):
    peptide_counts = df.groupby(['Protein.Names', 'Sample'])['Stripped.Sequence'].transform('nunique')
...
input_file = 'TABLES/report_updated.csv'
output_file = 'TABLES/report_updated_PROTEIN_ABUNDANCE_DATA_MANIPULATION_MODULE.csv'
...
pivot_df = df.pivot_table(
    index=['Protein.Names', 'Protein.Ids', 'Genes'],
    columns='Sample',
...
def on_button_click(b):
    with output:
        ...
        long_df = corrected_df.reset_index().melt(
            id_vars=['Protein.Names', 'Protein.Ids', 'Genes'],
            var_name='Sample',
            value_name='Genes.MaxLFQ.Unique'
        )
        ...
        cols = [col for col in final_df.columns if col != 'Genes.MaxLFQ.Unique']
        final_df = final_df[cols + ['Genes.MaxLFQ.Unique']]
...
primary_file = 'TABLES/report_updated_PROTEIN_ABUNDANCE_DATA_MANIPULATION_MODULE.csv'
fallback_file = 'TABLES/report_updated.csv'
...
#@title **Pivoted table for coefficients of variation (CVs) in experimental groups**
import pandas as pd
df = pd.read_csv('TABLES/Quantitative_parameters.csv')
df = df.drop_duplicates(subset=['Protein name', 'Protein IDs', 'Experimental groups', 'Genes'])
...
"""## **MS2 data points across peak with DIA-NN output**
It is essential to also include the **report.stats.tsv** file from DIA-NN ouputs in the working directory for calculation of MS2 data points.

Cycle Time is calculated as follows (input file **report.stats.tsv**):

- **Cycle Time = FWHM.RT*60/FWHM.Scans**

**FWHM** is calculated using the retention time values (input file **TABLES/report_updated.csv**):

- **FWHM = RT.Stop - RT.Start**

The number of points across a peak is determined using the following formula:

- **Points Across Peak = FWHM / Cycle Time**
...
stats_file_path = 'report.stats.tsv'
stats_df = pd.read_csv(stats_file_path, sep='\t')

stats_df['Cycle Time'] = (stats_df['FWHM.RT'] * 60) / stats_df['FWHM.Scans']
median_cycle_time = stats_df['Cycle Time'].median()

print(f"Median Cycle Time: {median_cycle_time:.3f} seconds")

report_file_path = 'TABLES/report_updated.csv'
df = pd.read_csv(report_file_path)

df.rename(columns={'Protein.Ids': 'Protein IDs', 'Protein.Names': 'Protein name'}, inplace=True)

df['RT'] = df['RT'] * 60
df['RT.Start'] = df['RT.Start'] * 60
df['RT.Stop'] = df['RT.Stop'] * 60


df['FWHM'] = df['RT.Stop'] - df['RT.Start']

df['Points Across Peak'] = df['FWHM'] / median_cycle_time
df['Points Across Peak'] = df['Points Across Peak'].replace([float('inf'), -float('inf')], pd.NA)

df_filtered = df[['Protein name', 'Protein IDs', 'Genes', 'Experimental groups', 'Points Across Peak']].copy()
df_filtered['Points Across Peak'] = pd.to_numeric(df_filtered['Points Across Peak'], downcast='float')
...
output_path = 'TABLES/points_across_peak_MS2.csv'
pivot_df.to_csv(output_path, index=False)
print(f"MS2 points across peak saved to '{output_path}'")
...
def run_analysis(b):
    with output:
        ...
        pivot_table = aggregated_df.pivot(index='Sample', columns='Experimental groups', values='Genes.MaxLFQ.Unique')
